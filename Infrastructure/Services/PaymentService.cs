...
namespace Infrastructure.Services
{
           ...
           IGenericRepository<DeliveryMethod> dmRepo) : IPaymentService
    {
        public async Task<ShoppingCart?> CreateOrUpdatePaymentIntent(string cartId)
        {
            ...
            var cart = await cartService.GetCartAsync(cartId); // اي حالات الحذف اصلا؟

            if (cart == null) return null;

            var shippingPrice = 0m;

            if (cart.DeliveryMethodId.HasValue) // اي لزمته وفين الالس ف حالة لو مفيش
            {
            ...
            foreach (var item in cart.Items)
            {
                ...
                if(item.Price != productItem.Price) // امته ده ممكن يحصل ان السعر يتغير اصلا قصدك ممكن هاك ف الفرونت ؟
                {
